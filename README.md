...
print(f"AI Response: {response.pydantic_message_parts} \n\n")
...
print(f"Web Research Tool response: {response.pydantic_message_parts} \n\n")
...
response = conversation.add_message(message)
print(f"Message with file response: {response.pydantic_message_parts} \n\n")
```
...
response = conversation.add_message(message)
print(f"Agent Response: {response.pydantic_message_parts}")
...
Add a remote MCP server pointing at the backend's `/mcp/` path (hosted:
`https://api.app.noxus.ai/mcp/`; self-hosted: `https://<host>/api/backend/mcp/`).
The Bearer token is your Noxus platform API key:

```json
{
  "mcpServers": {
    "noxus": {
      "url": "https://api.app.noxus.ai/mcp/",
      "headers": {
        "Authorization": "Bearer YOUR_NOXUS_API_KEY"
      }
    }
...
Set `execution_slots = 1` on your `BasePlugin` subclass (the default). Each slot
provides 1/8 CPU core and 256 MiB of memory. `noxus plugin generate-manifest`
includes the count and `noxus plugin validate` reports it. Regenerate and
reinstall after changing it. Installation uses at least 4 slots (1 GiB), then
shrinks to the declared count once the worker starts. A larger declared count
also increases the install budget. If an install or worker is killed for memory
use, raise `execution_slots` or reduce memory use in the reported step.
