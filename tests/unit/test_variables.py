...
def test_create_object_variable_sends_the_canonical_value_type() -> None:
    captured: dict = {}

    def handler(request: httpx.Request) -> httpx.Response:
        captured["body"] = _body(request)
        return httpx.Response(200, json={"id": "v-3", "name": "APP_CFG"})

    _client(handler).variables.create("APP_CFG", value='{"k": 1}', value_type="object")
    assert captured["body"]["value_type"] == "object"
    assert captured["body"]["inline_value"] == '{"k": 1}'
