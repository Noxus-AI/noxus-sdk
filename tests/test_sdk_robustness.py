    ...
    base = {
        ...
        **_BACKEND_EXTRA_SETTINGS_FIELDS,
        "tools": [
            ...
            _full_backend_tool("kb_qa", kb_id="kb-123", advanced_search=True, private=False),
            _full_backend_tool("workflow", workflow_id="wf-456", inputs_descriptions={}, version_id=None),
            _full_backend_tool("human_in_the_loop", has_timeout=False, timeout=None, handover_message="Transferring..."),
            ...
            _full_backend_tool("agent_tool", agent_id="agent-sub-1", max_clarification_turns=3),
...
class TestUnknownToolTypes:
    ...
    def test_unknown_tool_type_in_agent(self):
        ...
        payload = _full_backend_agent()
        payload["definition"]["tools"].append(
            _full_backend_tool("brand_new_tool")
        )
        agent = Agent(client=_client(), **payload)
    ...
    def test_unknown_tool_preserves_extra_fields(self):
        ...
        settings = _full_backend_settings()
        settings["tools"] = [
            {"type": "future_tool", "enabled": True, "extra_instructions": None,
             "custom_field": "custom_value", "nested": {"key": "val"}}
        ]
        s = ConversationSettings(**settings)
        tool = s.tools[0]
...
class TestToolEdgeCases:
    ...
    def test_tool_extra_instructions_as_string(self):
        s = ConversationSettings(
            ...
            temperature=0.7,
            tools=[
                {"type": "web_research", "extra_instructions": "Be thorough"}
            ],
        )
...
class TestConversationUpdateResponse:
    def test_refresh_updates_settings(self):
        ...
        refresh_response = _full_backend_conversation(
